...
The good news: the state hub pages (Texas, Colorado, etc.) are already pre-rendered with real content and correct tags, so the fix is proven and incremental — extend the same pre-rendering to all routes.
...
- **Fix:** static-generate (SSG) all routes at build time — Vite `vite-ssg`/prerender plugin, or migrate to Next.js. Every page should ship full HTML with its own title, description, canonical, H1, and body copy.
...
1. **Week 1:** P0-2 host alignment + P0-3 real 404s + P1-4 sitemap/robots fixes + P1-6 domain typo (small, config-level changes).
